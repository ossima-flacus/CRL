...
if (registrationForm) {
  registrationForm.addEventListener("submit", async (e) => {
    ...
    const formData = {
      ...
      birth_certificate_number: document.getElementById(
      ...
      enrollment_type: document.querySelector(
        'input[name="enrollment_type"]:checked',
      ).value,
      guardian: {
        ...
        profession:
          document.getElementById("guardian_profession").value || null,
      },
    };
    ...
    try {
      const response = await fetch(
        "../backend/public/index.php?route=register",
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok || !result.success) {
        showToast(
          result.message || "Erreur lors de l’envoi. Veuillez réessayer.",
          true,
        );
        submitButton.disabled = false;
        submitButton.innerHTML =
          '<i class="fas fa-paper-plane"></i> Envoyer la pré-inscription';
        return;
      }

      showToast(
        `✅ Inscription réussie ! Numéro d'inscription: ${result.registration_id}`,
      );
...
async function loadRegistrationClasses() {
  ...
  try {
    const response = await fetch(
      "../backend/public/index.php?route=class.list",
    );
