...
function buildClassesByLevel(classes, students) {
    ...
    const levels = ['Pré-primaire', 'Primaire', 'Lycée'];
    classesByLevel.innerHTML = levels.map((level) => {
        ...
        const classCards = levelClasses.map((cls) => {
...
function renderClassList(classes, students) {
    if (!classList) return;
    if (!classes.length) {
        classList.innerHTML = '<p class="text-slate-500">Aucune classe créée pour l’instant.</p>';
        return;
    }
...
function renderStudentList(students) {
    ...
    if (!students.length) {
        studentList.innerHTML = '<p class="text-slate-500">Aucun élève ajouté pour l’instant.</p>';
        return;
    }

    studentList.innerHTML = students.slice(0, 10).map((student) => {
...
function fillClassSelect(classes) {
    ...
    dashboardClassId.innerHTML = '<option value="">Sélectionnez une classe</option>';
    classes.forEach((cls) => {
...
async function createClass(data) {
    const response = await fetch(`${API_BASE}?route=class.create`, {
        method: 'POST',
...
async function createStudent(data) {
    const response = await fetch(`${API_BASE}?route=student.create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
    });
...
if (classForm) {
    classForm.addEventListener('submit', async (e) => {
        ...
        if (!result.success) {
            showDashboardToast(result.message || 'Erreur lors de la création de la classe.', true);
            return;
        }
...
if (studentForm) {
    studentForm.addEventListener('submit', async (e) => {
        ...
        const data = {
            ...
            email: document.getElementById('student_email') ? document.getElementById('student_email').value.trim() || null : null,
            phone: document.getElementById('student_phone') ? document.getElementById('student_phone').value.trim() || null : null,
            class_id: document.getElementById('dashboard_class_id').value,
            notes: document.getElementById('student_notes').value.trim() || null,
        };

        if (!data.first_name || !data.last_name || !data.gender || !data.birth_date || !data.class_id) {
            showDashboardToast('Veuillez remplir tous les champs obligatoires.', true);
            return;
        }

        const result = await createStudent(data);
        if (!result.success) {
            showDashboardToast(result.message || 'Erreur lors de l’ajout de l’élève.', true);
            return;
        }
