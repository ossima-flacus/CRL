...
function getStudentCountByClass(classId) {
  return students.filter(
    (student) => Number(student.class_id) === Number(classId),
  ).length;
}
...
function updateStatsAndViews() {
  ...
  if (avgFillRate) {
    const avg =
      classes.length === 0 ? 0 : (students.length / classes.length).toFixed(1);
...
function renderClassList() {
  ...
  if (classes.length === 0) {
    classListContainer.innerHTML =
      '<div class="empty-state"><i class="fas fa-chalkboard"></i> Aucune classe disponible.</div>';
    return;
  }

  classListContainer.innerHTML = classes
    .map((c) => {
...
function renderStudentList() {
  ...
  if (students.length === 0) {
    studentListContainer.innerHTML =
      '<div class="empty-state"><i class="fas fa-user-friends"></i> Aucun élève inscrit.</div>';
    return;
  }
  ...
  const html = levels
    .map((level) => {
      ...
      const studentCount = students.filter((s) =>
        classes.some(
          (c) => Number(c.id) === Number(s.class_id) && c.level === level,
        ),
      ).length;
...
function renderStructure() {
  ...
  const levels = ["Pré-primaire", "Primaire", "Lycée"];
  structureByLevel.innerHTML = levels
    .map((level) => {
