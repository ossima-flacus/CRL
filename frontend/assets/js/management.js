...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderClasses(classes, students) {
    ...
    if (!levelClasses.length) {
      classesList.innerHTML =
        '<p class="text-slate-600">Aucune classe créée pour ce niveau. Créez une classe pour afficher le détail.</p>';
      return;
    }

    classesList.innerHTML = levelClasses
      .map((cls) => {
        const count = students.filter(
          (student) => Number(student.class_id) === Number(cls.id),
        ).length;
        return `
  ...
  async function submitClass(event) {
    ...
    const description = document
      .getElementById("class_description")
      .value.trim();
  ...
  async function submitStudent(event) {
    ...
    const firstName = document
      .getElementById("student_first_name")
      .value.trim();
